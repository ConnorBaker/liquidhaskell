...
- Pin a third arming shape of that crash in tests: a dictionary function applied
  to a value argument before the refined dictionary, as an instance with an
  `Ord k` context in front of a refined `Semigroup v` produces
  [#1693](https://github.com/ucsd-progsys/liquidhaskell/issues/1693)
...
- Keep an unpacked field's binder, sort and refinement when expanding a data
