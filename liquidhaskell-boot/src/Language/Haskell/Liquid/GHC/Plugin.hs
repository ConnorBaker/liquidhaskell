...
loadDependencies currentModuleConfig mods = do
  ...
  totality  <- SpecFinder.findTotalitySpec hscEnv currentModuleConfig
  -- REVIEW: What does reversing the list accomplishes here?
  let deps = TargetDependencies $ foldl' processResult mempty (reverse results <> totality)
...
makeTargetSrc cfg file modGuts hscEnv rdrEnv methodSpans instanceSpans = do
  ...
  debugLog $ "derVars  => " ++ (O.showSDocUnsafe . O.ppr $ HS.fromList (LH.derivedVars cfg methodSpans mgiModGuts))
  ...
  return (TargetSrc
    ...
    , giDerVars   = HS.fromList (LH.derivedVars cfg methodSpans mgiModGuts)
